...
export async function GET(req: Request) {
  try {
    ...
    // Query: per person per day, sum of hours
    let sql = `
      SELECT
        u.${uId} AS person_id,
    ...
    type DayRecord = { date: string; hours: number };
    type PersonRecord = {
      person_id: string;
      display_name: string;
    ...
    // Ensure all whitelisted users appear even if they have zero rows
    // (the LEFT JOIN should produce at least one row per user with NULL work_date)

    // De-dupe by normalized display name (same logic as dept-person-month summary)
